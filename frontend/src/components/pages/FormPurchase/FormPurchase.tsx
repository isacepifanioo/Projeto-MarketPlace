...
export const FormPurchase = () => {
    ...
    async function postComment(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
          ...
          await InstacieAxios.post(`/review/${id}`, formData, {
            headers: {
    ...
    function handleDeleteImg(positionParam: number) {
      const fileItens = data.reviewFile?.filter((value) => {
        if(value instanceof File && data.reviewFile?.[positionParam] instanceof File) {
           return data.reviewFile[positionParam].name !== value.name
        }
      })
      setData((prevent) => ({ ...prevent, ["reviewFile"]: fileItens }));
    }

    return (
        <form onSubmit={postComment} style={{textAlign: "center", display: "flex", flexDirection: "column", gap: "1em"}}>
            ...
            <div>
                <textarea name="comment" id="comment" onChange={(e) => handleOnChageValue(e.target.value, e.target.name)} style={{width: "60%", height: "200px", fontSize: "1.2em", padding: ".6em"}} placeholder="Digite seu comentario"></textarea>
            </div>
